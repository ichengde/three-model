 ...
 */
import { DefaultLoadingManager, LoaderUtils, FileLoader, Vector3, VectorKeyframeTrack, EventDispatcher, Quaternion, Matrix4, MeshPhongMaterial, MeshLambertMaterial, MeshBasicMaterial, RepeatWrapping, ClampToEdgeWrapping, PerspectiveCamera, OrthographicCamera, DirectionalLight, PointLight, SpotLight, AmbientLight, BufferGeometry, Bone, Group, Skeleton, LineBasicMaterial, DoubleSide, Color, Float32BufferAttribute, LineSegments, Line, SkinnedMesh, Mesh, AnimationClip, Scene, TextureLoader, TGALoader, Euler } from 'three';
		...
		function parseKinematicsTransform(xml) {
			...
			switch (data.type) {
				...
				case 'rotate':
					data.obj = new Vector3();
					data.obj.fromArray(array);
					data.angle = TTMath.degToRad(array[3]);
					break;
							...
							for (var i = 0; i < transforms.length; i++) {
								...
								if (transform.sid && transform.sid.indexOf(jointIndex) !== - 1) {
									...
									switch (joint.type) {
										...
										case 'revolute':
											matrix.multiply(m0.makeRotationAxis(axis, TMath.degToRad(value)));
											break;
			...
			for (var i = 0; i < xml.childNodes.length; i++) {
				...
				switch (child.nodeName) {
					...
					case 'rotate':
						var array = parseFloats(child.textContent);
						var vector = new Vector3().fromArray(array);
						var angle = TMath.degToRad(array[3]);
						transforms.push({
							sid: child.getAttribute('sid'),
							type: child.nodeName,
							obj: vector,
